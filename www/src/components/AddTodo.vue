<template>
  <div class="add-todo toDo">
    ...
    <div class="userResults">
      <p v-for="todo in userTodos">{{todo.title}}
        <button @click="addTodo(todo)">Other users todo</button>
      </p>
    </div>
    <div class="googleResults">
      <p v-for="todo in googleTodos">{{todo.name}}
        <button @click="addGoogleTodo(todo)">Googles to do</button>
      </p>
    </div>
  </div>
</template>
